...
$this->send_order_admin($post_id,$post_type);
 ...
 $action= $send_to_sf_button ? '' : 'update';
  ...
  $res=array("class"=>"error","msg"=>__("Nothing Posted to Quickbooks",'wp-woocommerce-quickbooks'));      
  }
...
$fields=array("fields"=>$this->post('fields',$meta_post));
 ...
 if(!empty($info['meta']) ){
  ...
  $res['data']=$data;   
    ...
    $data['crm_sel_dis']=$this->gen_select($discount,$owner_sel,__('Select Discount Account','wp-woocommerce-quickbooks'));
    $meta['discount_accounts']=$discount; 
